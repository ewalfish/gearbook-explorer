...
export function validateAsset(input) {
    ...
    for (const [file, rows, kind] of [
        ...
        rows.forEach((raw, i) => {
            ...
            if (kind === 'lens') {
                    ...
                    if (!Array.isArray(tr) || tr.length === 0) {
                        ...
                        for (const t of tr) {
                            if (!TRAITS.includes(t))
                                add(file, line, 'traits.enum', `trait ${JSON.stringify(t)} is not in the vocabulary`);
                            if (seen.has(String(t)))
                                add(file, line, 'traits.duplicate', `trait ${JSON.stringify(t)} listed twice`);
                            seen.add(String(t));
                        }
                        // Sorted on the wire so a diff of two asset builds shows real
                        // changes, not reordering.
                        const sorted = [...tr].map(String).sort();
                        if (sorted.join(' ') !== tr.map(String).join(' ')) {
                ...
                const hasTrait = Array.isArray(tr) && tr.includes('folding');
    ...
    for (const raw of input.redirects ?? []) {
        ...
        if (!redirectSources.has(k))
            redirectSources.set(k, new Set());
        redirectSources.get(k).add(String(r?.to_id));
    }
    for (const [k, targets] of redirectSources) {
        if (targets.size > 1)
            add('redirects', 0, 'redirect.ambiguous', `${k} redirects to ${targets.size} different records`);
    }
