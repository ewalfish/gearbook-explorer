...
export class SearchEngine {
    ...
    search(raw, limit = 10) {
        ...
        for (const c of candidates) {
            ...
            if (!prev || this.compare(c, prev) < 0)
                best.set(c.entry.id, c);
        }
    ...
    compare(a, b) {
        ...
        if (ca !== cb)
            return ca - cb;
        if (a.entry.k !== b.entry.k)
            return a.entry.k === 'camera' ? -1 : 1;
        return (ma?.name ?? '').localeCompare(mb?.name ?? '');
    }
    collect(tokens, sourceLabel, tierOffset, out) {
        const q = tokens.join(' ');
        ...
        for (const tok of tokens) {
            ...
            if (matches === null)
                return; // token matches nothing — no cover possible
    ...
    entriesWithTokenPrefix(tok) {
        ...
        for (let i = lo; i < this.vocab.length; i++) {
            ...
            for (const idx of this.postings.get(v)) {
                out.add(idx);
                if (out.size > 30000)
                    return out; // extremely broad token; cap the union
            }
        }
    ...
    correctedQueries(tokens, hadLiteralHits) {
        const out = [];
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (hadLiteralHits && this.hasVocabPrefix(tok))
                continue;
...
export function damerauBounded(a, b, max) {
    if (a === b)
        return 0;
...
export function buildEngineInputs(entries, catalogRows, confidences) {
    const meta = new Map();
    for (const row of catalogRows) {
        meta.set(row[0], {
            name: row[2],
            confidence: confidences?.get(row[0]) ?? (row[5] ?? 'm'),
        });
    }
    return {
