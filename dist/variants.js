...
const TOKEN_SWAPS = (() => {
    ...
    for (const b of brands)
        out[b] = [...marketTokenSwaps(b), ...(SPELLING_SWAPS[b] ?? [])];
    return out;
})();
...
function brandOf(low) {
    for (const b of BRANDS)
        if (low.includes(b))
            return b;
    if (/\basahi\b/.test(low))
        return 'pentax';
    return null;
}
...
export function lensVariants(query) {
    ...
    if (/\bf\d|1:\d|\bf\/\s/i.test(base))
        add(base.replace(/\bf(\d)/gi, 'f/$1').replace(/\b1:(\d)/g, 'f/$1').replace(/f\/\s+/g, 'f/'));
    // "aus Jena" export engraving = Carl Zeiss Jena (post-war trademark-dispute marking)
    if (/\baus\s+jena\b/i.test(low))
        add(base.replace(/\b(carl\s+zeiss\s+)?aus\s+jena\b/i, 'Carl Zeiss Jena'));
    ...
    for (const [brandRe, lineRe, ins] of LENS_LINE)
        if (brandRe.test(low) && !lineRe.test(low))
            add(base.replace(brandRe, ins));
    ...
    if (/\bhelios\b(?!\s*-?\d)/i.test(low)) {
        if (/\b58\s*mm|\b58\b/i.test(low))
            add(base.replace(/\bhelios\b/i, 'Helios-44'));
        if (/\b85\s*mm|\b85\b/i.test(low))
            add(base.replace(/\bhelios\b/i, 'Helios-40'));
    }
    // Olympus OM lenses: sellers say "OM Zuiko"; the gearbook says "OM-System Zuiko"
    if (/\bom\s+zuiko\b/i.test(low))
        add(base.replace(/\bom\s+zuiko\b/i, 'OM-System Zuiko'));
    // Leica Noctilux/Summilux/Summicron M-mount: records carry the -M suffix
    if (/\b(noctilux|summilux|summicron|elmarit)\s+\d/i.test(low))
        add(base.replace(/\b(noctilux|summilux|summicron|elmarit)\b/i, '$1-M'));
    // Canon FD generations: FD ⇄ FDn ⇄ New FD are the same optical family name-wise
    if (/\bfdn?\b/i.test(low)) {
        add(base.replace(/\bfd\b/i, 'FDn'));
        add(base.replace(/\bfdn\b/i, 'FD'));
        add(base.replace(/\bnew fd\b/i, 'FDn'));
    }
    ...
    for (const v of [...out]) {
        if (/s\.s\.c\.?/i.test(v))
            add(v.replace(/s\.s\.c\.?/i, 'SSC'));
        else if (/\bssc\b/i.test(v))
            add(v.replace(/\bssc\b/i, 'S.S.C.'));
    }
    for (const v of [...out]) {
        if (/f\/?\s*\d\.0\b/i.test(v))
            add(v.replace(/(f\/?\s*\d)\.0\b/gi, '$1'));
        else
            add(v.replace(/(f\/?\s*\d)\b(?!\.)/gi, '$1.0'));
    }
    ...
    const dn = base.replace(/["""]/g, ' ').replace(/\([^)]*\)/g, ' ')
        ...
        .replace(/\s+/g, ' ').trim();
    if (dn.toLowerCase() !== low)
        add(dn);
    return [...out].slice(0, 18);
}
export function queryVariants(brand, model) {
    ...
    if (/sputnik/i.test(low))
        add(base.replace(/sputnik/gi, 'Spoutnik'));
    // Exacta/Exakta spelling drift (both appear in period literature and records)
    if (/exacta/i.test(low))
        add(base.replace(/exacta/gi, 'Exakta'));
    else if (/exakta/i.test(low))
        add(base.replace(/exakta/gi, 'Exacta'));
    // Nikonos records are named without the Nikon prefix; the original
    // 1963 body is plain "Nikonos" (composed: both fixes at once)
    if (/nikonos/i.test(low))
        add(base.replace(/\bnikon\b\s*/i, '').replace(/\bnikonos\s+i\b/i, 'Nikonos'));
    ...
    if (/\brollei/i.test(low))
        add(base.replace(/\b(xenotar|planar|xenar|tessar|opton|carl zeiss|cz|schneider)\b\s*/gi, ' ').replace(/\s+/g, ' ').trim());
    // glued marketing names: "Sureshot" ⇄ "Sure Shot"
    if (/\bsureshot\b/i.test(low))
        add(base.replace(/\bsureshot\b/i, 'Sure Shot'));
    ...
    if (/\bvpk\b/i.test(low))
        add(base.replace(/\b(kodak\s+)?vpk\b/i, 'Vest Pocket Kodak'));
    ...
    if (/\bminca\b/i.test(low) && /\b(c2|c3)\b/i.test(low))
        add(base.replace(/\b(argus\s+)?minca\b/i, 'Argus'));
    // "Golden Shield by Argus" = Sylvania-era (1959+) rebadged Argus C3
    if (/\bgolden\s+shield\b/i.test(low))
        add(base.replace(/\bgolden\s+shield(\s+by)?\b/i, ' ').replace(/\bargus\b\s*(c3)?/i, 'Argus C3').replace(/\s+/g, ' ').trim() || 'Argus C3');
    ...
    if (/\bargus\b/i.test(low)) {
        ...
        if (noNick.toLowerCase() !== low)
            add(noNick);
        if (/\bc21\b/i.test(low))
            add(noNick.replace(/\bc21\b/i, '21'));
    }
    ...
    if (/super\s*ikonta/i.test(low)) {
        const SI = { a: '531', b: '532/16', bx: '533/16', c: '531/2', d: '530/15' };
    ...
    for (const grp of [['fujifilm', 'fujica', 'fuji'], ['konica minolta', 'minolta']]) {
        ...
        if (hit)
            for (const alt of grp)
                if (alt !== hit)
                    add(base.replace(new RegExp(`\\b${hit}\\b`, 'i'), alt));
    }
    ...
    if (bk) {
        ...
        const spaced = (s) => s.replace(/([a-z])(\d)/gi, '$1 $2').replace(/(\d)([a-z]{1,3})\b/gi, '$1 $2');
        for (const [a, b] of TOKEN_SWAPS[bk] ?? []) {
            const ra = new RegExp(`\\b${a}\\b`, 'i'), rb = new RegExp(`\\b${b}\\b`, 'i');
            if (ra.test(flat)) {
                const v = flat.replace(ra, b);
                add(v);
                add(spaced(v));
            }
            if (rb.test(flat)) {
                const v = flat.replace(rb, a);
                add(v);
                add(spaced(v));
            }
        }
        ...
        const applyMap = (m) => {
            for (const [k, v] of Object.entries(m)) {
                ...
                if (re.test(flat))
                    for (const vv of [].concat(v))
                        add(flat.replace(re, vv).replace(/\b(\w+)\s+\1\b/gi, '$1'));
            }
        };
        applyMap(map);
        applyMap(inv);
    ...
    if (bk === 'pentax' && /spotmatic|\bsp ?(ii|f|500|1000)\b/i.test(low)) {
        if (/\bsp ?ii\b|\bspii\b/i.test(low))
            add(base.replace(/\bsp ?ii\b|\bspii\b/i, 'II'));
        if (/\bsp ?f\b|\bspf\b/i.test(low))
            add(base.replace(/\bsp ?f\b|\bspf\b/i, 'F'));
        if (/spotmatic\s+sp\b/i.test(low))
            add(base.replace(/\s+sp\b/i, ''));
    }
    ...
    if (bk === 'olympus') {
        ...
        for (const v of [...out]) {
            ...
            if (/mju/i.test(v))
                add(v.replace(/\b(stylus|epic|infinity)\b\s*/gi, ''));
            if (/stylus|infinity/i.test(v) && /mju/i.test(v) === false)
                add(v.replace(/\b(stylus|infinity)\b\s*/gi, 'mju '));
            ...
            if (!/infinity/i.test(v))
                add(v.replace(/olympus/i, 'Olympus Infinity'));
    ...
    if (bk === 'canon') {
        ...
        for (const v of [...out]) {
            if (/\beos\b/i.test(v))
                add(v.replace(/\beos\b\s*/i, ''));
            else if (eosBody.test(v))
                add(v.replace(/\bcanon\b/i, 'Canon EOS'));
        }
    }
