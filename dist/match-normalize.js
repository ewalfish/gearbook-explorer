...
export function matchNormalize(s) {
    ...
    t = t.split(' ').map((tok, idx, arr) => ROMAN[tok] && (tok.length > 1 || idx === arr.length - 1) ? ROMAN[tok] : tok).join(' ');
    return t;
}
export const matchTokens = (s) => matchNormalize(s).split(' ').filter(Boolean);
export const digitSeqs = (s) => matchNormalize(s).match(/\d+(\.\d+)?/g) ?? [];
//# sourceMappingURL=match-normalize.js.map