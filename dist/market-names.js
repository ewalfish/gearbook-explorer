...
export const MARKET_ORDER = ['intl', 'us', 'eu', 'jp'];
/**
...
export function marketTokenSwaps(brand) {
    const out = [];
 ...
 */
export function marketModelMap(brand) {
    const map = {};
    const flat = (s) => s.replace(/-/g, ' ');
    const add = (from, to) => { (map[flat(from)] ||= []).push(flat(to)); };
...
//# sourceMappingURL=market-names.js.map