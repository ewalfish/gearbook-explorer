 ...
 */
const DEAD_FILM: Record<string, { label: string; detail: string }> = {
...
export function hazards(data: HazardInput | null | undefined): Hazard[] {
  const out: Hazard[] = []
  if (!data) return out
...
export const hasHazard = (data: HazardInput | null | undefined): boolean => hazards(data).length > 0
