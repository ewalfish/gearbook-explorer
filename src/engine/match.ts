...
function stripDescriptors(query: string): string | null {
  const out = query.replace(DESCRIPTORS, ' ').replace(/\s+/g, ' ').trim()
  if (!out || out === query.trim()) return null
  ...
  if (words.length < 2 && !/[\d-]/.test(out)) return null
  return out
}
...
export function matchOne(
  ...
  if (decision === 'no-match' && depth === 0) {
    ...
    const subs: { q: string, safe: boolean }[] = []
    if (stripped) subs.push({ q: stripped, safe: true })
    for (const s of marketSplits(stripped ?? query)) subs.push({ q: s, safe: false })
    if (stripped) for (const s of marketSplits(query)) subs.push({ q: s, safe: false })
    for (const { q: sub, safe } of subs) {
      ...
      if (r.best && qualConflict(query, r.best.entry.title)) continue
      if (r.decision !== 'no-match' && (!alt || (r.best?.s ?? 0) > (alt.best?.s ?? 0))) { alt = r; altSafe = safe }
    }
    if (alt?.best) {
