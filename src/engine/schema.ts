      ...
      if (kind === 'lens') {
        ...
        if (ms !== undefined) {
          if (!Array.isArray(ms) || ms.length === 0) {
            ...
            if (new Set(ms.map(String)).size !== ms.length) add(file, line, 'mounts.duplicate', 'a mount is listed twice')
            for (const m of ms as unknown[]) {
              if (!isStr(m)) add(file, line, 'mounts.value', 'mount entries must be non-empty strings')
              // A single entry containing a comma means a SET leaked back in as
              // a string — the exact defect this field exists to retire.
              else if (String(m).includes(',')) add(file, line, 'mounts.joined', `mount ${JSON.stringify(m)} is a joined list, not one mount`)
            }
            ...
            if (data.mount !== undefined && data.mount !== (ms as string[]).join(', ')) {
              add(file, line, 'mount.derived-mismatch', `mount ${JSON.stringify(data.mount)} is not mounts.join(', ')`)
            }
          }
        }
      } else {
        if ((data as Record<string, unknown>).mounts !== undefined) add(file, line, 'mounts.on-camera', 'mounts is a lens field; a body has one lens_mount')
